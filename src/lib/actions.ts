...
export async function exportCellar() {
    ...
    try {
        ...
        // CSV Header
        const header = ["Name", "Producer", "Vintage", "Type", "Region", "Country", "Quantity", "Price", "Note"];
        const rows = items.map(item => [
            item.wine.name.replace(/"/g, '""'), // Escape quotes
            item.wine.producer.replace(/"/g, '""'),
            item.wine.vintage || "",
            item.wine.type,
            (item.wine.region || "").replace(/"/g, '""'),
            (item.wine.country || "").replace(/"/g, '""'),
            item.quantity,
            item.buyPrice || "",
            // Note: Simplification (no notes in this query yet, typically in TastingNotes)
            ""
        ].map(field => `"${field}"`).join(",")); // Quote all fields

        const csvContent = [header.join(","), ...rows].join("\n");
        return { success: true, csv: csvContent, filename: `ma-cave-${new Date().toISOString().split('T')[0]}.csv` };
...
export async function importCellar(formData: FormData) {
    const session = await auth();
    if (!session?.user?.id) return { success: false, message: "Non authentifié" };
    ...
    try {
        ...
        // Basic CSV Parsing (assuming standard Quotes+Comma format generated by our export)
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const [name, producer, vintageStr, typeStr, region, country, qtyStr, priceStr] = cols;

            if (!name || !producer) continue;

            const vintage = parseInt(vintageStr) || null;
            const quantity = parseInt(qtyStr) || 1;
            const price = parseFloat(priceStr) || null;
            // Map Type
            let type = "OTHER";
            const upperType = (typeStr || "").toUpperCase();
            if (["RED", "WHITE", "ROSE", "SPARKLING"].includes(upperType)) type = upperType;

            // Find or Create Wine
            let wine = await prisma.wine.findFirst({
                where: { name, producer, vintage }
            });

            if (!wine) {
                wine = await prisma.wine.create({
                    data: { name, producer, vintage, type, region, country }
                });
            }
        ...
        revalidatePath('/dashboard/cellar');
        return { success: true, message: `${successCount} vins importés avec succès !` };

    } catch (e) {
        console.error("Import Error:", e);
        return { success: false, message: "Erreur lors de l'import. Vérifiez le format CSV." };
    }
}
